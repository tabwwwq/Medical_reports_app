...
WHERE d.IsActive = 1
  AND (d.Specialization = 'General Practitioner (GP)' OR d.Specialization = 'Врач общей практики (ВОП)' OR d.Specialization = 'General Practitioner (GP)')
...
ORDER BY d.FirstName, d.LastName;";
        ...
        private bool IsGeneralPractitioner(MySqlConnection connection, MySqlTransaction transaction, int doctorId)
        {
            using (MySqlCommand command = new MySqlCommand(@"
SELECT COUNT(*)
FROM Doctors
WHERE DoctorId = @DoctorId
  AND IsActive = 1
  AND (Specialization = 'General Practitioner (GP)' OR Specialization = 'Врач общей практики (ВОП)' OR Specialization = 'General Practitioner');", connection, transaction))
            {
        ...
        private bool IsGeneralPractitioner(MySqlConnection connection, int doctorId)
        {
            return IsGeneralPractitioner(connection, null, doctorId);
        }
        ...
        public void SendRequest(int patientId, int doctorId)
        {
            ...
            {
                connection.Open();
                if (!IsGeneralPractitioner(connection, doctorId))
                {
                    throw new Exception("Only a General Practitioner (GP) can be selected as a family doctor.");
                }
...
ORDER BY r.RequestDate DESC, r.RequestId DESC;";
        ...
        public void AcceptRequest(int requestId, int doctorId)
        {
            ...
            using (MySqlConnection connection = data.GetConnection())
            {
                ...
                {
                    if (!IsGeneralPractitioner(connection, transaction, doctorId))
                    {
                        throw new Exception("Only a General Practitioner (GP) can accept family doctor requests.");
                    }
                    int patientId;
