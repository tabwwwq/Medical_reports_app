...
WHERE p.IsActive = 1
  AND p.FamilyDoctorId = @DoctorId
  AND (@SearchText = '' OR CONCAT(IFNULL(p.FirstName, ''), ' ', IFNULL(p.LastName, '')) LIKE CONCAT('%', @SearchText, '%'))
GROUP BY p.PatientId, p.FirstName, p.LastName, p.BirthDate, p.AvatarUrl
            ...
            using (MySqlCommand command = new MySqlCommand(query, connection))
            {
                ...
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        visit.DoctorSpecialization = reader.IsDBNull(reader.GetOrdinal("Specialization")) ? "General doctor" : reader.GetString("Specialization");
