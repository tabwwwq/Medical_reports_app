...
namespace MedicalReportsApp.Services
{
    public class DoctorDashboardService
    {
        ...
        public Doctor GetDoctorById(int doctorId)
        {
            new AvatarService().EnsureAvatarColumns();
            string query = @"
SELECT DoctorId, FirstName, LastName, Email, PasswordHash, Gender, BirthDate, Specialization, CreatedAt, IsActive, AvatarUrl
FROM Doctors
...
WHERE p.IsActive = 1
  AND (
      @SearchText = ''
      OR CONCAT(IFNULL(p.FirstName, ''), ' ', IFNULL(p.LastName, '')) LIKE CONCAT('%', @SearchText, '%')
      OR CONCAT(IFNULL(p.LastName, ''), ' ', IFNULL(p.FirstName, '')) LIKE CONCAT('%', @SearchText, '%')
      OR IFNULL(p.Email, '') LIKE CONCAT('%', @SearchText, '%')
      OR IFNULL(p.Phone, '') LIKE CONCAT('%', @SearchText, '%')
  )
GROUP BY p.PatientId, p.FirstName, p.LastName, p.BirthDate, p.AvatarUrl, p.FamilyDoctorId
ORDER BY CASE WHEN p.FamilyDoctorId = @DoctorId THEN 0 ELSE 1 END,
         CASE WHEN MAX(v.VisitDate) IS NULL THEN 1 ELSE 0 END,
         MAX(v.VisitDate) DESC,
         p.FirstName,
         p.LastName;";
            ...
            using (MySqlCommand command = new MySqlCommand(query, connection))
            {
                ...
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        card.AvatarUrl = reader.IsDBNull(reader.GetOrdinal("AvatarUrl")) ? "" : reader.GetString("AvatarUrl");
        ...
        {
            string query = @"
SELECT PatientId, FirstName, LastName, Email, PasswordHash, Address, City, BloodType, Gender, BirthDate, Phone, FamilyDoctorId, CreatedAt, IsActive, AvatarUrl
FROM Patients
WHERE PatientId = @PatientId AND IsActive = 1;";
            ...
            using (MySqlCommand command = new MySqlCommand(query, connection))
            {
                ...
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    ...
                    patient.AvatarUrl = reader.IsDBNull(reader.GetOrdinal("AvatarUrl")) ? "" : reader.GetString("AvatarUrl");
                    return patient;
                }
            }
        }
...
ORDER BY cdt.Name;";
        ...
        {
            string query = @"
SELECT d.FirstName, d.LastName, d.Specialization, v.VisitDate, v.VisitType, v.Diagnosis, v.Notes
FROM Visits v
            ...
            using (MySqlCommand command = new MySqlCommand(query, connection))
            {
                ...
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        visit.DoctorSpecialization = reader.IsDBNull(reader.GetOrdinal("Specialization")) ? "General Practitioner (GP)" : reader.GetString("Specialization");
...
LIMIT 50;";
            ...
            using (MySqlCommand command = new MySqlCommand(query, connection))
            {
                ...
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        prescription.PrescriptionId = reader.GetInt32("PrescriptionId");
                        prescription.PatientId = reader.GetInt32("PatientId");
                        prescription.DoctorId = reader.IsDBNull(reader.GetOrdinal("DoctorId")) ? null : (int?)reader.GetInt32("DoctorId");
                        prescription.Name = reader.GetString("Name");
                        prescription.MedicineName = prescription.Name;
                        prescription.Dosage = reader.GetString("Dosage");
                        prescription.Quantity = reader.GetString("Quantity");
                        prescription.Status = reader.IsDBNull(reader.GetOrdinal("Status")) ? "Active" : reader.GetString("Status");
                        prescription.CreatedAt = reader.GetDateTime("CreatedAt");
                        string doctorFirstName = reader.IsDBNull(reader.GetOrdinal("FirstName")) ? "" : reader.GetString("FirstName");
                        string doctorLastName = reader.IsDBNull(reader.GetOrdinal("LastName")) ? "" : reader.GetString("LastName");
                        string doctorName = (doctorFirstName + " " + doctorLastName).Trim();
                        prescription.DoctorName = string.IsNullOrWhiteSpace(doctorName) ? "Not specified" : "Dr. " + doctorName;
        ...
        public void SavePatientRecord(int patientId, int doctorId, string firstName, string lastName, DateTime visitDate, string reason, string diagnosis, string notes, List<string> allergies, List<string> chronicProblems, List<PrescriptionEditorItem> prescriptions)
        {
            ...
            using (MySqlConnection connection = data.GetConnection())
            {
                ...
                using (MySqlTransaction transaction = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        SavePrescriptions(connection, transaction, patientId, doctorId, prescriptions);
...
VALUES (@PatientId, @AllergyTypeId, @DoctorId);", connection, transaction))
        ...
        private void SaveChronicProblems(MySqlConnection connection, MySqlTransaction transaction, int patientId, int doctorId, List<string> chronicProblems)
        {
            ...
            foreach (string rawName in chronicProblems)
            {
                ...
                int chronicId = EnsureLookupId(connection, transaction, "ChronicDiseaseTypes", "ChronicDiseaseTypeId", name);
                using (MySqlCommand insertCommand = new MySqlCommand(@"
...
VALUES (@PatientId, @ChronicDiseaseTypeId, @DoctorId);", connection, transaction))
                {
                    insertCommand.Parameters.AddWithValue("@PatientId", patientId);
                    insertCommand.Parameters.AddWithValue("@ChronicDiseaseTypeId", chronicId);
                    insertCommand.Parameters.AddWithValue("@DoctorId", doctorId);
                    insertCommand.ExecuteNonQuery();
                }
            }
...
VALUES (@PatientId, @DoctorId, @VisitDate, @VisitType, @Diagnosis, @Notes);", connection, transaction))
            {
                ...
                insertCommand.Parameters.AddWithValue("@VisitType", string.IsNullOrWhiteSpace(reason) ? "General visit" : reason.Trim());
        ...
        private void SavePrescriptions(MySqlConnection connection, MySqlTransaction transaction, int patientId, int doctorId, List<PrescriptionEditorItem> prescriptions)
        {
            using (MySqlCommand deleteCommand = new MySqlCommand("DELETE FROM Prescriptions WHERE PatientId = @PatientId;", connection, transaction))
            {
                deleteCommand.Parameters.AddWithValue("@PatientId", patientId);
                deleteCommand.ExecuteNonQuery();
            }

            foreach (PrescriptionEditorItem item in prescriptions)
            {
                string name = (item.Name ?? "").Trim();
                string dosage = (item.Dosage ?? "").Trim();
                ...
                using (MySqlCommand insertCommand = new MySqlCommand(@"
INSERT INTO Prescriptions (PatientId, DoctorId, Name, Dosage, Quantity, Status, CreatedAt)
VALUES (@PatientId, @DoctorId, @Name, @Dosage, @Quantity, @Status, @CreatedAt);", connection, transaction))
                {
                    insertCommand.Parameters.AddWithValue("@PatientId", patientId);
                    insertCommand.Parameters.AddWithValue("@DoctorId", doctorId);
                    insertCommand.Parameters.AddWithValue("@Name", name);
                    insertCommand.Parameters.AddWithValue("@Dosage", dosage);
        ...
        private void WritePatientAuditLogs(MySqlConnection connection, MySqlTransaction transaction, PatientAuditSnapshot beforeSnapshot, int doctorId, Doctor doctor, string firstName, string lastName, DateTime visitDate, string reason, string diagnosis, string notes, List<string> allergies, List<string> chronicProblems, List<PrescriptionEditorItem> prescriptions)
        {
            ...
            string newPrescriptions = auditLogService.JoinList(ToAuditPrescriptionList(prescriptions));
        ...
        private List<string> ToAuditPrescriptionList(List<PrescriptionEditorItem> prescriptions)
        {
...
ORDER BY cdt.Name;", connection, transaction))
            ...
            using (MySqlCommand command = new MySqlCommand(@"
SELECT CONCAT(Name, ' - ', Dosage, ' - ', Quantity) AS PrescriptionText
FROM Prescriptions
WHERE PatientId = @PatientId
ORDER BY CreatedAt DESC, PrescriptionId DESC;", connection, transaction))
            {
                command.Parameters.AddWithValue("@PatientId", patientId);
                using (MySqlDataReader reader = command.ExecuteReader())
                {
