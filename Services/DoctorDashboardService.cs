...
SELECT p.PatientId,
       ...
       COALESCE(
            (
                ...
                WHERE pcd.PatientId = p.PatientId
                ORDER BY cdt.Name
                LIMIT 1
            ),
            'No condition added'
            ...
            using (MySqlCommand command = new MySqlCommand(query, connection))
            {
                ...
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        card.Condition = reader.IsDBNull(reader.GetOrdinal("ConditionName")) ? "No condition added" : reader.GetString("ConditionName");
...
VALUES (@PatientId, @ChronicDiseaseTypeId, @DoctorId);", connection, transaction))
        ...
        {
            using (MySqlCommand deleteCommand = new MySqlCommand("DELETE FROM Visits WHERE PatientId = @PatientId AND VisitDate = @VisitDate;", connection, transaction))
            {
                deleteCommand.Parameters.AddWithValue("@PatientId", patientId);
                deleteCommand.Parameters.AddWithValue("@VisitDate", visitDate.Date);
                deleteCommand.ExecuteNonQuery();
            }
