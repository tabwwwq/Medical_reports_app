            ...
            using (MySqlCommand command = new MySqlCommand(query, connection))
            {
                ...
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        ...
                        dashboard.DoctorSpecialization = reader.IsDBNull(reader.GetOrdinal("Specialization")) ? "General Practitioner (GP)" : reader.GetString("Specialization");
                    }
                }
            }
        }
...
LIMIT 50;";
        ...
        public bool VerifyPatientPassword(string email, string password)
        {
            ...
            return PasswordHelper.VerifyPassword(password, savedHash);
        }
