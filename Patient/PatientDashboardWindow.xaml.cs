...
namespace MedicalReportsApp
{
    public partial class PatientDashboardWindow : Window
    {
        ...
        private void LoadDashboard()
        {
            ...
            currentPatient = dashboard.Patient;

            LoadPatientAvatar(dashboard.Patient);
            ...
            {
                btnFindDoctor.Visibility = Visibility.Visible;
                btnMessageDoctor.Visibility = Visibility.Collapsed;
                btnRemoveDoctor.Visibility = Visibility.Collapsed;
            }
            else
            {
                btnFindDoctor.Visibility = Visibility.Collapsed;
                btnMessageDoctor.Visibility = Visibility.Visible;
                btnRemoveDoctor.Visibility = Visibility.Visible;
            }
        ...
        private void LoadPatientAvatar(Patient patient)
        {
            ImageBrush brush = avatarService.BuildAvatarBrush(patient.AvatarUrl);
            if (brush == null)
            {
                patientAvatarBorder.Background = new SolidColorBrush((Color)ColorConverter.ConvertFromString("#FFE5E7EB"));
                txtPatientAvatarIcon.Visibility = Visibility.Visible;
            }
            else
            {
                patientAvatarBorder.Background = brush;
                txtPatientAvatarIcon.Visibility = Visibility.Collapsed;
            }
        }
        ...
        private void btnFindDoctor_Click(object sender, RoutedEventArgs e)
        {
            ...
            FindDoctorWindow window = new FindDoctorWindow(currentPatient.Id);
            window.Owner = this;
            window.ShowDialog();
            LoadDashboard();
        }

        private void btnMessageDoctor_Click(object sender, RoutedEventArgs e)
        {
            if (currentPatient == null || currentPatient.FamilyDoctorId == null)
            {
                MessageBox.Show("You need a family doctor before you can send messages.", "Messages", MessageBoxButton.OK, MessageBoxImage.Information);
                return;
            }

            ChatWindow window = new ChatWindow(currentPatient.Id, currentPatient.FamilyDoctorId.Value, "Patient");
            window.Owner = this;
            window.ShowDialog();
        }
        ...
        private void btnRemoveDoctor_Click(object sender, RoutedEventArgs e)
        {
            ...
            MessageBoxResult result = MessageBox.Show("Remove your current family doctor? After this you can send a new request to another doctor.", "Remove Doctor", MessageBoxButton.YesNo, MessageBoxImage.Question);
            ...
            try
            {
                FamilyDoctorService familyDoctorService = new FamilyDoctorService();
                familyDoctorService.RemoveFamilyDoctor(currentPatient.Id);
                MessageBox.Show("Doctor was removed. Now you can send a new request.", "Done", MessageBoxButton.OK, MessageBoxImage.Information);
                LoadDashboard();
            }
            catch (Exception ex)
            {
